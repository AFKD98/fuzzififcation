...
def gen_randomcodes(funcnum=1):
  for i in range(len(DELAYS)):
    ...
    while True:
      ...
      capt = subprocess.getoutput(cmd2)
      num_if = int(capt)
...
def bin_compile(name):
  ...
  output = base
  subprocess.getoutput("gcc %s -o %s >/dev/null 2>&1" % (name, output))

def lib_compile(item, ranstr):
  base = os.path.basename(item).split(".c")[0]
  output = WORKING_DIR +"/" + base + ".o"
  subprocess.getoutput("gcc -c %s -o %s >/dev/null 2>&1" % (item, output))
